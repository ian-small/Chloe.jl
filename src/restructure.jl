...
function rotate!(target::FwdRev{CircularSequence}, result::ChloeAnnotation, fstart::Integer)::Tuple{FwdRev{CircularSequence},ChloeAnnotation}
...
function flip!(target::FwdRev{CircularSequence}, result::ChloeAnnotation, fliprange::UnitRange{<:Integer})::Tuple{FwdRev{CircularSequence},ChloeAnnotation}
    gl = result.target_length
    if length(fliprange) == gl
        ...
        new_fseq = LongDNA{2}() * target.forward[1:fliprange.start-1] * BioSequences.reverse_complement(target.forward[fliprange]) * target.forward[fliprange.stop+1:gl]
        new_fseq_mask = CircularMask(vcat(target.forward.mask[1:fliprange.start-1], reverse(target.forward.mask[fliprange]), target.forward.mask[fliprange.stop+1:gl]))
        new_rseq = LongDNA{2}() * target.reverse[1:rc(fliprange.stop+1,gl)] * BioSequences.reverse_complement(target.reverse[rc(fliprange, gl)]) * target.reverse[rc(fliprange.start-1, gl):gl]
        new_rseq_mask = CircularMask(vcat(target.reverse.mask[1:rc(fliprange.stop+1, gl)], reverse(target.reverse.mask[rc(fliprange, gl)]), target.reverse.mask[rc(fliprange.start-1, gl):gl]))
        target = FwdRev(CircularSequence(new_fseq, new_fseq_mask), CircularSequence(new_rseq, new_rseq_mask))
        flipped_forward_annotations = filter(x -> length(circularintersect(gene_span(x), fliprange, gl)) > 0, result.annotation.forward) #should correctly handle genes that start or finish in flipped region
        ...
        flipped_reverse_annotations = filter(x -> length(circularintersect(gene_span(x), rc(fliprange, gl), gl)) > 0, result.annotation.reverse)
        ...
        new_fwd_annotations = append!(filter(x -> x ∉ flipped_forward_annotations, result.annotation.forward), flipped_reverse_annotations)
        new_rev_annotations = append!(filter(x -> x ∉ flipped_reverse_annotations, result.annotation.reverse), flipped_forward_annotations)
        result = ChloeAnnotation(result.target_id, result.target_length, result.coverages, FwdRev(new_fwd_annotations, new_rev_annotations))
    end
    target, result
end
...
function IR_range(annotation::Vector{SFF_Model})::Union{Nothing, UnitRange{Int}}
    ...
    if isnothing(IR_idx); return nothing; end
    IR = first(annotation[IR_idx].features).feature
    IR.start:IR.start + IR.length - 1
end

function restructure!(target::FwdRev{CircularSequence}, result::ChloeAnnotation, templates::Dict{String,FeatureTemplate})::Tuple{FwdRev{CircularSequence},ChloeAnnotation}
    gl = result.target_length
    ...
    if isnothing(IRa_range)
        ...
        IRa_annotations = filter(x -> length(circularintersect(gene_span(x), IRa_range, gl)) > 0, result.annotation.forward)
        if check_orientation(IRa_annotations, refstrands)
            ...
            #flip IR annotations
            IRa_feature = first(result.annotation.forward[findfirst(x -> startswith(x.gene, "IR"), result.annotation.forward)].features).feature
            IRb_feature = first(result.annotation.reverse[findfirst(x -> startswith(x.gene, "IR"), result.annotation.reverse)].features).feature
        ...
        LSC_annotations = filter(x -> length(circularintersect(gene_span(x), LSC_range, gl)) > 0, result.annotation.forward)
        ...
        SSC_annotations = filter(x -> length(circularintersect(gene_span(x), SSC_range, gl)) > 0, result.annotation.forward)
